...
- Antigravity User Settings で **Strict Mode** を有効にします。
  - *動作の内容:* ストリクトモードが有効な場合、ネットワークアクセスが拒否された状態でサンドボックスが自動的にアクティブになり、シェル実行の前に手動のユーザープロンプトを強制します。人間がすべてのコマンドを確認する必要があるため、`gh` CLI などのツールを使用した高度なデータ流出手法を効果的に軽減できます。
